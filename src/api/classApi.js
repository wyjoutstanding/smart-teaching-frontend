...
/**
 * 加入班级
 * @param {班级邀请码} classInviteCode 
 */
export function joinClass(classInviteCode) { //登录接口
    return request({ //使用封装好的 axios 进行网络请求
        url: '/api/classes/joinClass',
        method: 'get', // get与post写法不同，这里需要传递参数，而不是data
        params: { //提交的数据
            studentId: sessionStorage.getItem('id'), // 用户id
            classInviteCode
        }
    })
}
 ...
 * @param {班级id} classId 
 */
export function quitClass(classId) { //登录接口
    return request({ //使用封装好的 axios 进行网络请求
        url: '/api/classes/quitClass',
        method: 'get', // get与post写法不同，这里需要传递参数，而不是data
        params: { //提交的数据
            studentId: sessionStorage.getItem('id'), // 用户id
            classId
