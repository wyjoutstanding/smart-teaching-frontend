                        ...
                        <card class="text-left">
                            ...
                            <el-row class="row" type='flex' justify="space-around">
                                <el-button
                                    ...
                                    size="medium"
                                    round="round"
                                    @click="formClass.dialogFormVisible=true;classId=classes.id">修改班级</el-button>
                                <span></span>
                                <el-button
                                    type="danger"
                                    size="medium"
                                    round="round"
                                    @click="handleDeleteClass(classes.id)">删除班级</el-button>
                            </el-row>
                        </card>
                    </span>
                    <!-- <span v-for="o in 10" :key="o" class="col-md-4"> <card class="text-center">
                    <h4 class="card-title">{{classesCard.className}}</h4> <el-divider></el-divider>
                    <p class="card-text">班级类型：{{classesCard.classType}}</p> <p
                    class="card-text">创建时间：{{classesCard.createTime}}</p> <el-button type="warning"
                    size="medium" round>修改班级</el-button> <el-button type="danger" size="medium"
                    round>删除班级</el-button> </card> </span> -->
                    <!-- 修改班级对话框 -->
                    <el-dialog
                        title="修改班级"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                // classesCard:{   classId:'',  班级id,   className:'班级名',   classType:'班级类型',
                // classCreateTime:'创建时间',   classInviteCode:'班级邀请码' },
        ...
        methods: {
            ...
            handleDeleteClass(classId) { // 删除班级
                this
                    .$confirm('此操作将删除班级信息，是否继续？', '提示', { // 确认信息
            ...
            handleModifyClass() { // 修改班级
                this
                    .$confirm('此操作将修改班级信息，是否继续？', '提示', { // 确认信息
                        confirmButtonText: '确定', // 确认的文本标签
                        cancelButtonText: '取消',
                        type: 'warning'
                    })
